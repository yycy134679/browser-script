...
GM_addStyle(`
    ...
    #bookmark-manager-modal .bm-content-panel { max-width: 850px; }
    /* 头部 */
    ...
    /* 表格 */
    #bookmarks-table-container { min-height: 200px; max-height: 50vh; overflow-y: auto; flex-grow: 1; }
    ...
    /* WebDAV 浏览器 */
    #webdav-browser-modal .bm-content-panel { max-width: 600px; height: 70vh; }
    ...
    /* WebDAV 设置 */
    #webdav-settings-modal .bm-content-panel { max-width: 500px; }
...
const getRootTopicUrl = (url) => (url.match(/(https:\/\/linux\.do\/t\/[^\/]+\/\d+)/) || [])[0] || url;
...
function renderBookmarksTable(searchText = '') {
    ...
    const filteredBookmarks = allBookmarks.filter(bm => bm.name.toLowerCase().includes(lowerCaseSearch) || bm.url.toLowerCase().includes(lowerCaseSearch));
    if (filteredBookmarks.length === 0) { tableContainer.innerHTML = '<p style="text-align:center; color:#888; padding:20px 0;">没有找到匹配的收藏。</p>'; return; }
    let tableHTML = `<table id="bookmarks-table"><thead><tr><th>名称</th><th>链接</th><th style="text-align:center;">操作</th></tr></thead><tbody>`;
    filteredBookmarks.forEach(bookmark => {
        const originalIndex = allBookmarks.findIndex(b => getRootTopicUrl(b.url) === getRootTopicUrl(bookmark.url));
        tableHTML += `<tr data-original-index="${originalIndex}"><td>${bookmark.name}</td><td><a href="${bookmark.url}" target="_blank" title="${bookmark.url}">${bookmark.url}</a></td><td style="text-align:center;"><button class="bm-btn rename-btn">重命名</button><button class="bm-btn bm-btn-danger delete-btn">删除</button></td></tr>`;
    });
...
function handleLocalImport(file) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (!Array.isArray(importedBookmarks)) throw new Error('文件格式不正确。');
            promptAndMergeBookmarks(importedBookmarks);
        } catch (error) { alert('导入失败！请确保文件是正确的 JSON 格式备份文件。\n错误: ' + error.message); }
...
function handleLocalExport() {
    ...
    if (bookmarks.length === 0) { alert('没有收藏可以导出。'); return; }
    const dataStr = JSON.stringify(bookmarks, null, 2);
    const blob = new Blob([dataStr], {type: 'application/json'});
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = 'linuxdo_bookmarks_backup.json'; a.click(); URL.revokeObjectURL(url);
}

// --- Part 4: WebDAV 核心功能 ---
function getWebDAVConfig(fromStorage = true) {
    const server = fromStorage ? GM_getValue('webdav_server') : getEl('webdav-server').value.trim();
    const user = fromStorage ? GM_getValue('webdav_user') : getEl('webdav-user').value.trim();
    const pass = fromStorage ? GM_getValue('webdav_pass') : getEl('webdav-pass').value;
...
function saveWebDAVConfig() {
    ...
    if (!config) { alert('服务器、用户名和应用密码均不能为空！'); return; }
...
function webdavRequest(options) {
    ...
    if (!config) {
        if (options.onerror) options.onerror({ status: 0, statusText: 'WebDAV 配置不完整' });
    ...
    GM_xmlhttpRequest({
        ...
        headers: { 'Authorization': 'Basic ' + btoa(config.user + ':' + config.pass), ...options.headers },
        data: options.data,
...
function testWebDAVConnection() {
    const config = getWebDAVConfig(false);
    if (!config) { webdavTestResult.className = 'test-fail'; webdavTestResult.textContent = '❌ 请填写所有字段！'; return; }
    webdavTestResult.className = ''; webdavTestResult.textContent = '正在测试连接...';
    webdavRequest({
        method: 'PROPFIND', path: '', headers: { 'Depth': '0' },
        onload: (res) => {
            if (res.status === 207 || res.status === 200) { webdavTestResult.className = 'test-success'; webdavTestResult.textContent = '✅ 连接成功！'; }
            else if (res.status === 401) { webdavTestResult.className = 'test-fail'; webdavTestResult.textContent = '❌ 连接失败: 用户名或密码错误 (401)'; }
            else { webdavTestResult.className = 'test-fail'; webdavTestResult.textContent = `❌ 连接失败: 服务器返回 ${res.status}`; }
        },
        onerror: () => { webdavTestResult.className = 'test-fail'; webdavTestResult.textContent = '❌ 连接失败: 请检查服务器地址或网络'; }
    });
}
function uploadToWebDAV(isAuto = false) {
    ...
    const performPut = () => {
        webdavRequest({
            method: 'PUT', path: WEBDAV_BACKUP_DIR + filename,
            headers: { 'Content-Type': 'application/json; charset=utf-8' },
            data: JSON.stringify(bookmarks, null, 2),
            onload: (res) => {
                if (res.status === 201 || res.status === 204) { showToast(`✅ ${isAuto ? '自动' : '手动'}备份成功！`); }
                else { showToast(`❌ 备份失败: ${res.status} ${res.statusText}`, true); }
            },
            onerror: (res) => showToast(`❌ 备份出错: ${res.statusText}`, true)
        });
    };
    webdavRequest({
        method: 'MKCOL', path: WEBDAV_BACKUP_DIR,
        onload: (res) => {
            if (res.status === 201 || res.status === 405 || res.status === 409) { performPut(); }
            else { showToast(`❌ 创建云端目录失败: ${res.status} ${res.statusText}`, true); }
        },
        onerror: (res) => showToast(`❌ 创建云端目录出错: ${res.statusText}`, true)
    });
}
...
function listWebDAVBackups() {
    ...
    webdavBrowserList.innerHTML = '<li class="loading-text">正在加载备份列表...</li>';
    webdavRequest({
        method: 'PROPFIND', path: WEBDAV_BACKUP_DIR, headers: { 'Depth': '1' },
        onload: (res) => {
            if (res.status !== 207) {
            ...
            const xmlDoc = parser.parseFromString(res.responseText, "application/xml");
            const files = Array.from(xmlDoc.getElementsByTagName('d:href'))
                .map(node => node.textContent.split('/').pop())
                .filter(name => name.startsWith('linuxdo-backup-') && name.endsWith('.json'))
                .sort().reverse();
            if (files.length === 0) { webdavBrowserList.innerHTML = '<li class="loading-text">云端没有找到任何备份文件。</li>'; return; }
        ...
        onerror: (res) => webdavBrowserList.innerHTML = `<li class="loading-text">加载出错: ${res.statusText}</li>`
    });
}
function downloadFromWebDAV(filename) {
    alert(`即将从云端恢复备份: ${filename}`);
    webdavRequest({
        method: 'GET', path: WEBDAV_BACKUP_DIR + filename,
        onload: (res) => {
            if (res.status === 200) {
                try {
                    const cloudBookmarks = JSON.parse(res.responseText);
                    if (!Array.isArray(cloudBookmarks)) throw new Error('云端数据格式错误。');
                    webdavBrowserModal.style.display = 'none';
                    promptAndMergeBookmarks(cloudBookmarks);
                } catch (e) { alert('解析云端数据失败！\n错误: ' + e.message); }
            } else { alert(`下载失败！服务器响应: ${res.status} ${res.statusText}`); }
        },
        onerror: (res) => alert(`下载出错！请检查网络、服务器地址和授权。\n详情: ${res.statusText}`)
    });
}

// --- Part 5: 通用逻辑与事件绑定 ---
function promptAndMergeBookmarks(newBookmarks) {
    const choice = prompt("请选择恢复模式：\n1. 增量合并 (智能去重)\n2. 完全覆盖 (清空本地后恢复)\n\n请输入数字 1 或 2");
    let dataChanged = false;
    modifyBookmarks(bookmarks => {
        if (choice === '1') {
            const currentUrls = new Set(bookmarks.map(b => getRootTopicUrl(b.url)));
            let addedCount = 0;
            newBookmarks.forEach(b => { if (b.url && !currentUrls.has(getRootTopicUrl(b.url))) { bookmarks.push(b); addedCount++; } });
            if (addedCount > 0) dataChanged = true;
            alert(`合并完成！新增 ${addedCount} 条，跳过 ${newBookmarks.length - addedCount} 条重复项。`);
        } else if (choice === '2') {
...
document.body.addEventListener('click', function(event) {
    ...
    if (row) { const originalIndex = parseInt(row.dataset.originalIndex, 10); if (target.classList.contains('delete-btn')) deleteBookmark(originalIndex); else if (target.classList.contains('rename-btn')) enterEditMode(row); else if (target.classList.contains('save-btn')) saveRename(originalIndex, row.querySelector('.edit-name-input').value); else if (target.classList.contains('cancel-btn')) renderBookmarksTable(searchInput.value); return; }
    const buttonActions = {
        'manage-bookmarks-button': () => { renderBookmarksTable(); managerModal.style.display = 'flex'; },
        'webdav-settings-btn': () => {
    ...
    if (target.classList.contains('bm-close-btn')) getEl(target.dataset.targetModal).style.display = 'none';
    if (target.classList.contains('bm-modal-backdrop') && !event.target.closest('.bm-content-panel')) target.style.display = 'none';
});
fileInput.addEventListener('change', (e) => e.target.files[0] && handleLocalImport(e.target.files[0]));
searchInput.addEventListener('input', () => renderBookmarksTable(searchInput.value));
autoSyncToggle.addEventListener('change', (e) => GM_setValue('webdav_auto_sync_enabled', e.target.checked));
...
if (window.location.href.includes('linux.do/t/')) {
    ...
    collectButton.addEventListener('click', () => {
        ...
        const customName = prompt('请输入收藏名称（默认为帖子标题）:', cleanTitle);
        if (customName) {
            modifyBookmarks(bookmarks => {
...
console.log('超级收藏夹 (v3.8 终极稳定版) 已加载！');
})();